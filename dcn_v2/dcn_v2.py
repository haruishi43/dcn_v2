...
try:
    from dcn_v2 import dcn_backend as _backend
except:
...
class _DCNv2Pooling(Function):
    ...
    def forward(
        ...
        output, output_count = \
            _backend.dcn_v2_psroi_pooling_forward(
                input, rois, offset,
    ...
    def backward(ctx, grad_output):
        ...
        grad_input, grad_offset = \
            _backend.dcn_v2_psroi_pooling_backward(
                grad_output,
                input,
                rois,
                offset,
    ...
    def __init__(
        ...
        if not no_trans:
            self.offset_mask_fc = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Linear(
                    self.deform_fc_dim, self.pooled_size *
                    self.pooled_size * 3)
            )
