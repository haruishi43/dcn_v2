...
void modulated_deformable_col2im_cpu_kernel(
    const int n, const float *data_col, const float *data_offset, const float *data_mask,
...
void modulated_deformable_col2im_coord_cpu_kernel(
  ...
  for(int index = 0; index < n; index++) {
    ...
    const float *data_mask_ptr = data_mask + (b * deformable_group + deformable_group_index) * \
